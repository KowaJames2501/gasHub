...
export default function GasHubDashboard() {
  ...
  const fetchAgents = async () => {
    try {
      ...
      const res = await fetch(`${BASE_URL}/api/getagents`, { headers });
  ...
  const fetchData = async () => {
    try {
      ...
      triggerLocalNotification("Update Error", "Could not sync data.");
    } finally {
      setRefreshing(false);
    }
  };
    ...
    <View style={styles.mainWrapper}>
      ...
      <Animated.View style={[styles.drawer, { left: drawerAnim, width: drawerWidth }]}>
        <View style={styles.drawerHeader}>
          <MaterialCommunityIcons name="gas-cylinder" size={32} color="#FF3B30" />
        ...
        <View style={styles.drawerLinks}>
          <DrawerItem icon="view-dashboard" label="Overview" onPress={() => { toggleMenu(false); router.push("/homepage"); }} />
          <DrawerItem icon="clipboard-list" label="My Orders" onPress={() => { toggleMenu(false); router.push("/allorders"); }} />
          <DrawerItem icon="cart-plus" label="New Order" onPress={() => { toggleMenu(false); router.push("/customerorder"); }} />
      ...
      <View style={styles.container}>
        ...
        >
            ...
            {agents.length > 0 ? (
              agents.map((agent) => (
                <TouchableOpacity
                  ...
                  style={[styles.agentCard, selectedAgent?.id === agent.id && styles.selectedAgentCard]}
                >
                  <View style={styles.agentAvatarBox}>
                    <View style={styles.agentImgWrapper}>
                      {agent?.profile_url ? (
                        <Image source={{ uri: `${BASE_URL}/uploads/${agent?.profile_url}` }} style={styles.avatarImg} />
                      ) : (
                        <MaterialCommunityIcons name="account" size={30} color="#555" />
                      )}
                    </View>
                    <View style={[styles.statusDot, { backgroundColor: agent.status === "online" ? "#34C759" : "#FF9500" }]} />
                  </View>
                  <Text numberOfLines={1} style={styles.agentNameShort}>{agent.name?.split(" ")[0]}</Text>
                  <Text style={styles.agentStatusText}>{agent.status || "offline"}</Text>
                </TouchableOpacity>
              ))
            ) : (
              <Text style={styles.emptyText}>Finding agents...</Text>
            )}
          </ScrollView>
          {/* DYNAMIC INVENTORY */}
          {selectedAgent && (
            <View style={styles.inventoryContainer}>
              <View style={styles.inventoryHeader}>
                <Text style={styles.inventoryTitle}>{selectedAgent.name}'s Catalog</Text>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 15 }}>
                <Text style={styles.prodPrice}>{selectedAgent.phone}</Text>
                <Text style={styles.inventoryTitle}>{selectedAgent.location}</Text>
              </View>
              <View style={styles.productGrid}>
                {selectedAgent.products && selectedAgent.products.length > 0 ? (
                  selectedAgent.products.map((prod) => (
                    <View key={prod.id} style={styles.productItem}>
                      <View style={styles.prodIconBox}>
                         <MaterialCommunityIcons name="gas-cylinder" size={20} color="#FF3B30" />
                      </View>
          ...
          <View style={styles.grid}>
            <StatCard title="Active Agents" value={agents.filter(a => a.status === 'online').length} icon="moped" color="#007AFF" desc="Ready to deliver" />
            <StatCard title="Pending" value={pendingOrders} icon="clock-outline" color="#FF9500" desc="Orders in queue" />
            <StatCard title="Completed" value={completedOrders} icon="check-decagram" color="#34C759" desc="Successful deliveries" />
            <StatCard title="Total Orders" value={orders.length} icon="clipboard-text" color="#FF3B30" desc="Lifetime history" />
          </View>
...
const LogItem = ({ icon, text, time }) => (
  <View style={styles.logRow}>
    <MaterialCommunityIcons name={icon} size={18} color="#FF3B30" style={{ marginRight: 12 }} />
    <View style={{ flex: 1 }}>
...
const styles = StyleSheet.create({
  ...
  badge: { 
    ...
    backgroundColor: '#FF3B30', justifyContent: 'center', alignItems: 'center', borderWidth: 2, borderColor: '#000' 
  },
  ...
  selectedAgentCard: { borderColor: "#FF3B30", backgroundColor: "#1C1917" },
  ...
  prodPrice: { color: "#FF3B30", fontWeight: "800", fontSize: 12 },
  ...
  logoutText: { color: "#FF3B30", marginLeft: 10, fontWeight: "800" },
